...
const FilterListStaffLoader: DataLoaderItem = {
	path: "/:link",
	loader: async (path: string, params: any, url?: string) => {
		...
		const { dataListStaffAtHome, rawListStaffAtHome, pageType, breadcrumb, pageTitle, labelData, totalData } =
			listStaffResult;
...
export default FilterListStaffLoader;

export const FilterListStaffLoaderMuti: DataLoaderItem = {
	path: "/:lang(vi|en|kr|jp|cn)/:link",
	loader: async (path: string, params: any, url?: string) => {
		const lang = params.lang || "vi";
		const apartment = params.apartment;
		let start = new Date();
